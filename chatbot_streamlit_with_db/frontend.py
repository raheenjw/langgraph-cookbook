...
if 'chat_threads' not in st.session_state:
    #load chat threads from db
    st.session_state['chat_threads'] = retrieve_all_threads()
    # Initialize titles for existing threads
    if 'thread_titles' not in st.session_state:
        st.session_state['thread_titles'] = {}
    for tid in st.session_state['chat_threads']:
        hist = load_conversation_history(tid)
        if hist and isinstance(hist[0], HumanMessage):
            st.session_state['thread_titles'][tid] = hist[0].content
        else:
            st.session_state['thread_titles'][tid] = str(tid)
...
if user_input:
    ...
    with st.chat_message('assistant'):
        ...
        ai=st.write_stream(ai_only_stream())
        # ai=st.write_stream(
    ...
    current_title = st.session_state.get('thread_titles', {}).get(st.session_state.thread_id)
    if not current_title or current_title == str(st.session_state.thread_id):
